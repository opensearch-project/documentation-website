...
The following example demonstrates how to use the S3 source and Grok prepper plugins to process unstructured log data
from S3. This example uses Amazon Application Load Balancer (ALB) logs. As the ALB writes logs to S3, S3 creates notifications
in SQS. Data Prepper reads those notifications and reads the S3 objects to get the log data and process it.
