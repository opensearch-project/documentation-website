...
CSV reports have a non-configurable 10,000 row limit up to version 2.16. As of version 2.17, this limit can be configured when setting up a report. While reports have no explicit size limit (for example, MB), extremely large documents could cause report generation to fail with an out of memory error from the V8 JavaScript engine.
{: .tip }
