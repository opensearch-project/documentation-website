...
For information about working with existing detection rules, see [Creating detection rules]({{site.url}}{{site.baseurl}}/security-analytics/usage/rules/#creating-detection-rules).
...
1. In the **Detection** section, select a log type and review the detection rules and field mappings. Do the following:
    ...
    * Use the **Search** bar to search for specific rules.
    ...
    C. Review the field mappings. Field mappings allow the system to accurately pass event data from the log to the detector and then use the data to trigger alerts. For more information about field mappings, see the **About field mappings** section later in this topic.
