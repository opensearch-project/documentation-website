...
1. In the **Data source** section, select one or more sources for the log data. Use an asterisk (*) to indicate a wildcard pattern. When selecting multiple data sources, their logs must be of the same type. We recommend creating separate detectors for different log types. Security Analytics also supports the following:

  - [Aliases]({{site.url}}{{site.baseurl}}/im-plugin/index-alias/). When configuring an alias as data source, it must be attached to a **Write** index alias. Furthermore, when using an alias, ensure that your documents are ingesting through the alias and **not** the index(es) the alias was created for.
  - [Data streams]({{site.url}}{{site.baseurl}}/im-plugin/data-streams/) for time-series generated data.
