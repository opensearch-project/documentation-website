...
`inputs` | Object | Detector inputs for its definition.
`inputs.detector_input` | Array | An array that contains the index and definition used to create the detector. For the initial release in 2.4, only one log data source is allowed for the detector. Support for associating multiple log data sources with a single detector will be available in a future release. See description for `inputs.detector_input.indices` and the log data source.
