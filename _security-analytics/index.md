...
The correlation engine accomplishes this by using correlation rules to define threat scenarios involving different log types. It can then perform queries on logs to make matches between relevant findings from those different log sources. To depict relationships between events occuring in different logs, a correlation graph provides a visual representation of findings, their connections, and how close those connections are. While the correlation rules define what threat scenarios to look for between log types, the graph provides a visualization that helps users identify the relationship between different findings in a chain of security events.
