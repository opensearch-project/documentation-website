...
`prevent_empty_rollover` | Controls whether to skip the rollover when the index contains no documents. When `true`, an empty index does not roll over. Defaults to `false`. | Boolean | `true` or `false` | No
...
### Grouping rollover conditions

**Introduced 3.7**
{: .label .label-purple }

Conditions set directly on the `rollover` object are combined with a logical OR, so the rollover occurs as soon as one of them is met. To require that several conditions be met together, use `any_of`.

`any_of` takes a list of condition groups. The conditions within a group are combined with AND, and the groups are combined with OR, so the rollover occurs when every condition in at least one group is met. Each group is an object containing one or more of `min_size`, `min_primary_shard_size`, `min_doc_count`, and `min_index_age`, which follow the same rules and units as in the preceding table.

Parameter | Description | Type | Required
:--- | :--- |:--- |:--- |
`any_of` | A list of condition groups. Within a group, the conditions are combined with AND; the groups are combined with OR. | Array | No
...
`any_of` and the conditions set directly on the `rollover` object are mutually exclusive. Specifying both, an empty `any_of` list, or an empty group returns an error.
