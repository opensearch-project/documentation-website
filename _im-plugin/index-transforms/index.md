...
- *Groups* place documents into buckets in the target index. For example, grouping the `DestAirportID` field of the sample flight data produces a `DestAirportID_terms` field in the target index, with one bucket per airport. Transform jobs support the `histogram`, `date_histogram`, and `terms` [bucket aggregations]({{site.url}}{{site.baseurl}}/aggregations/bucket/index/).
...
After the transform job finishes, search the target index with the `_search` API. Each document in the target index contains the grouped fields, the aggregated values, and a `transform._doc_count` field with the number of source documents in the bucket.

The following request returns the buckets of a transformed flight index in which `DestAirportID_terms` is `SFO`:
...
To reach the **Index Management** page, go to **Management > Index Management** on the top menu. Select **Transform jobs** to list the transform jobs in your cluster with their source index, target index, and status. Select a job to see its configuration and the results of its runs. To enable, disable, or delete jobs, select the checkbox next to each one, select **Actions**, and then select the operation.

If your cluster has no data to transform, add the sample flight data from the OpenSearch Dashboards home page and transform that.
...
1. Select **Next**.
1. In **Define transform**, select the fields to summarize:
...
1. In **Specify schedule**, do the following:
   ...
   1. To transform the buckets that change after each run, select **Continuous**.
   1. In **Transform execution interval**, enter an interval and select **Minutes**, **Hours**, or **Days**.
