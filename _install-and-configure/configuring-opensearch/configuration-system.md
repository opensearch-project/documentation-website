...
- `bootstrap.system_call_filter` (Static, Boolean): Controls whether OpenSearch asks the operating system to block the system calls that create a new process. Set it to `false` only on a platform where the filter cannot be installed. For the mechanism used on each platform and the conditions under which a failed installation stops the node, see [Blocking process creation at the operating system level]({{site.url}}{{site.baseurl}}/scripting/script-security/#blocking-process-creation-at-the-operating-system-level). Default is `true`.
