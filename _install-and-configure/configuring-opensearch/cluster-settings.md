...
- `cluster.routing.allocation.rebalance.primary.buffer` (Dynamic, floating point): Defines how far a node's primary shard count may exceed the cluster average before the node is considered unbalanced during rebalancing. A node is considered unbalanced when its primary shard count reaches `ceil(average_primaries_per_node * (1 + buffer))`. For example, in a cluster averaging 300 primary shards per node, a buffer of `0.1` results in a threshold of 330. This setting applies to the rebalancing phase only: shard allocation always uses the cluster average as the threshold, regardless of this setting. Requires `cluster.routing.allocation.rebalance.primary.enable` to be enabled. Default is `0.1`.
