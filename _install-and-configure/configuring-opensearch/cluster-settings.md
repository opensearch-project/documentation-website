...
- `cluster.routing.allocation.rebalance.primary.buffer` (Dynamic, floating point): Sets the primary shard threshold, relative to the cluster average, at which rebalancing treats a node as unbalanced. A node is unbalanced when its primary shard count reaches `ceil(average_primaries_per_node * (1 + buffer))`; for example, in a cluster averaging 300 primary shards per node, a buffer of `0.1` sets the threshold at 330. This setting affects rebalancing only; shard allocation always applies the same formula with a buffer of `0`, that is, `ceil(average_primaries_per_node)`. Applies when `cluster.routing.allocation.rebalance.primary.enable` is enabled. Default is `0.1`.
