...
- `cluster.routing.allocation.rebalance.primary.buffer` (Dynamic, floating point): Sets how far a node's primary shard count may rise above the cluster average before rebalancing treats the node as unbalanced. The average is the total number of primary shards divided by the number of data nodes in the cluster, including data nodes that currently hold no shards, such as nodes excluded by allocation filters. A node is unbalanced when its primary shard count reaches `ceil(average_primaries_per_node * (1 + buffer))`. For example, a cluster with 3,000 primary shards across 20 data nodes has an average of 150, so a buffer of `0.1` sets the threshold at 165. This setting affects rebalancing only; shard allocation applies the same formula with a buffer of `0`. Applies when `cluster.routing.allocation.rebalance.primary.enable` is enabled. Default is `0.1`.
