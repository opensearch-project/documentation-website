...
- `cluster.fault_detection.leader_check.timeout` (Time unit): Determines the timeout duration for each node, indicating the period it waits for a response from the elected cluster manager during a leader check before deeming it as a failure. It can now be set dynamically through Update cluster settings API. Changing this setting from the default value can result into unstable cluster. Possible value can range from `1ms` to `60s` with default being `10s`.

- `cluster.fault_detection.follower_check.timeout` (Time unit): Determines the timeout duration for the elected cluster manager, specifying the duration it waits for a response during a follower check before deeming it as a failure. It can now be set dynamically through Update cluster settings API. Changing this setting from the default value can result into unstable cluster. Possible value can range from `1ms` to `60s` with default being `10s`.
