...
- `cluster.routing.allocation.rebalance.primary.buffer` (Dynamic, floating point): Sets how far a node's primary shard count may rise above the cluster average before the node is deprioritized as a target for relocated shards. The threshold is `ceil(average_primaries_per_node * (1 + buffer))`, and a node is deprioritized when its primary shard count reaches that value. The average is the total number of primary shards divided by the number of data nodes in the cluster, including data nodes that currently hold no shards, such as nodes excluded by allocation filters. For example, a cluster with 3,000 primary shards across 20 data nodes has an average of 150, so a buffer of `0.1` sets the threshold at 165. This setting applies to shard relocation, both rebalancing and moving shards off nodes where they can no longer remain, and takes effect when `cluster.routing.allocation.rebalance.primary.enable` is `true`. Allocation of unassigned shards applies the same formula with a buffer of `0` when `cluster.routing.allocation.balance.prefer_primary` is `true`. Default is `0.1`.
