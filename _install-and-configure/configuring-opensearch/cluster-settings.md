...
- `cluster.routing.allocation.rebalance.primary.enable` (Boolean): When set to `true`, OpenSearch attempts to evenly rebalance the primary shards between the cluster nodes. Enabling this setting, cluster tries to maintain number of primary shards on each node with maximum buffer defined by `cluster.routing.allocation.rebalance.primary.buffer`. Changing this setting to `false` after it was set to `true` does not invoke redistribution of primary shards. Default is `false`.
