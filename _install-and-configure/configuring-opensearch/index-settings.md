...
- `cluster.default_number_of_replicas` (Integer): Controls the default number of replicas for indexes. `index.number_of_replicas` will pick up this value if not configured. Default is 1.
...
- `index.number_of_replicas` (Integer): The number of replica shards each primary shard should have. For example, if you have 4 primary shards and set `index.number_of_replicas` to 3, the index has 12 replica shards. It picks up `cluster.default_number_of_replicas` as its default value and if `cluster.default_number_of_replicas` is not set, default is 1.
