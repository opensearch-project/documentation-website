...
- `index.number_of_search_replicas` (Integer): The number of search replica shards each primary shard should have. For example, if you have 4 primary shards and set `index.number_of_search_replicas` to 3, the index has 12 search replica shards. If not set, defaults to 0.
...
- `index.auto_expand_search_replicas` (String): Specifies whether the cluster should automatically adjust the number of search replica shards based on the number of available search nodes. The value should be provided as a range, with a lower and upper bound — for example, `0-3` or `0-all`. For example, if you have 5 data nodes and set `index.auto_expand_search_replicas` to `0-3`,  the index can have up to 3 search replicas and the cluster does not automatically add another search replica shard. However, if you set this value to `0-all` and add 2 more nodes for a total of 7, the cluster will expand to now have 7 search replica shards. Default is disabled.
