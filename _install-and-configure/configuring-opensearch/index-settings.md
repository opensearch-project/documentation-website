...
- `index.sort.field` (String): Specifies the field used to sort documents at index time. The default sort order is ascending. To change the order, set the `index.sort.order` parameter.

- `index.sort.order` (String): Specifies the sort order for documents at index time. Valid options are `asc` (ascending) and `desc` (descending). Default is `asc`. This setting requires `index.sort.field` to be set.
