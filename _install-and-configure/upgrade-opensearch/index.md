...
[Segment replication]({{site.url}}{{site.baseurl}}/tuning-your-cluster/availability-and-recovery/segment-replication/index/) could help you continue to index data on your primary shard as you being a rolling upgrade especially if your cluster has a high index count and lower replica shard count. Enabling segment replication gives you the ability upgrade nodes without need to disable than reenable replica shards, allowing indexing to continue even as you perform a rolling upgrade. For instructions about how to enable segment replication, see [Segment replication configuration]({{site.url}}{{site.baseurl}}/tuning-your-cluster/availability-and-recovery/segment-replication/index/#segment-replication-configuration)
