...
The field of Vector Search contains a significant number of tradeoffs. Some users want very low-latency search, others want low-cost search. The `mode` mapping parameter of the `knn_vector` type allows users to indicate what tradeoffs they would like to default to. The `mode` that is set will dictate how default parameters are resolved. Users can fine tune their index by overriding other parameters in the k-NN field mapping.
