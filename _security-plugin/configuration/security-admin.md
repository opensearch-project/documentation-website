...
`securityadmin.sh` requires that SSL/TLS transport is enabled on your opensearch cluster. Make sure that the `plugins.security.ssl.http.enabled: true` is set in `opensearch.yml` before proceeding.
{: .note}
