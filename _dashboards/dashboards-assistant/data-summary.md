...
To configure data summary, use the following steps.

### Prerequisite

Before using data summary, enable query enhancements in OpenSearch Dashboards as follows:
...
### Step 1: Enable data summary

To enable data summary, configure the following `opensearch_dashboards.yml` setting:
...
To orchestrate data summary, create an `os_query_assist_ppl` agent. To create an agent, send a `POST /_plugins/_ml/agents/_register` request and provide the agent template as a payload:
...
:--- | :--- | :---
`sample_data` | Required | A sample of data returned by the specified query, used as input for summarization.
`question` | Optional | The user's natural language question about the data, which guides the summary generation.
`ppl` | Optional | The Piped Processing Language (PPL) query to retrieve data; in query assistance, this is generated by the LLM from the user's natural language question.
...
1. In the query language dropdown list, select **PPL**. You will see the generated data summary below the query text, as shown in the following image.
