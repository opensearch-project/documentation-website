...
### Rescoring support for hybrid queries
Introduced 2.18
{: .label .label-purple }
...
1. Each sub-query in the hybrid query executes on the shard, producing separate result sets.
2. The rescore query is applied to each sub-query's results independently.
