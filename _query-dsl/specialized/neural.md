...
`k` | Integer | Optional | The number of results returned by the k-NN search. Only one variable, either `k`, `min_score` or `max_distance`, can be specified. If none of `k`, `min_score` or `max_distance` variable is specified, `k` with value `10` is used by default.
`min_score` | Float | Optional | The minimum score threshold for the search results. Only one variable, either `k`, `min_score` or `max_distance`, can be specified. See [k-NN radial search]({{site.url}}{{site.baseurl}}/search-plugins/knn/radial-search-knn/) for more information.
`max_distance` | Float | Optional | The maximum distance threshold for the search results. Only one variable, either `k`, `min_score` or `max_distance`, can be specified. See [k-NN radial search]({{site.url}}{{site.baseurl}}/search-plugins/knn/radial-search-knn/) for more information.
