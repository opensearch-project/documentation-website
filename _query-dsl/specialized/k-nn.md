...
`rescore` | Object or Boolean | Optional | Parameters for configuring rescoring functionality:<br>- `oversample_factor` (Float): Controls the oversampling of candidate vectors before ranking. Valid values are in the `[1.0, 100.0]` range. Default is `false` for fields with `mode=in_memory`(no rescoring) and enabled (with dynamic values) when `mode=on_disk`. See [on disk default table](https://opensearch.org/docs/latest/field-types/supported-field-types/knn-memory-optimized/#rescoring-quantized-results-to-full-precision) for more information. To use the default `oversample_factor` of `1.0`, set `rescore` to `true`. For more information, see [Rescoring results](#rescoring-results).
