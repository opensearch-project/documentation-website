...
The default encoding algorithm for signing the JWT is HMAC SHA512. Both keys are base64-encoded strings in [security `config.yml` file]({{site.url}}{{site.baseurl}}/security/configuration/configuration/). After the configuration is applied using `securityadmin.sh -cd <configuration directory>` command, the values are stored in the security system index and used cluster-wide.
