...
`roles_key` | The key in the JSON payload that stores the user's roles. The value of this key must be a comma-separated list of roles. Required only if you want to use roles in the JWT. `roles_key` can be configured as a list to extract roles from nested JWT claims.
