...
Due to security restrictions, the `keytab` and the `krb5.conf` files must be placed in `config` or a subdirectory, and the path in `opensearch.yml` must be relative, not absolute.
{: .note }
