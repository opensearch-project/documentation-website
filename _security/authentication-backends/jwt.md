...
`required_audience` | The name of the audience which the JWT must specify. You can set single or multiple values by using the `,` delimeter such as `project1` or `project1,admin`. If you set multiple values, the JWT must have at least one required audience. This corresponds [`aud` claim of the JWT](https://datatracker.ietf.org/doc/html/rfc7519#section-4.1.3).
