...
- Each run deletes at most 50,000 memories per memory type per container (50k sessions + 50k long-term + 50k history working memory is not included). A larger backlog is reduced over successive runs, so a container that far exceeds its `max_count` may take several runs to reach the limit.
