...
- Each run deletes at most 50,000 memories per memory type, per container — applied independently to sessions, long-term, and history (working memory is not subject to this cap). A larger backlog is reduced over successive runs, so a container that far exceeds its `max_count` may take several runs to reach the limit.
