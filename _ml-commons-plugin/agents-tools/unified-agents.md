---
layout: default
title: Unified agent API (Experimental)
...
The unified agent API is disabled by default. To use it, you must enable the following cluster setting:
...
The unified agent API provides the following improvements over the traditional agent workflow:
...
### Unified workflow (3.5+)

With the unified agent API, you can register an agent in a single API call:
...
POST /_plugins/_ml/agents/_register
{
  ...
  "type": "conversational",
  "description": "A unified agent using Claude",
  "model": {
...
The unified agent API introduces a flexible `input` field that supports three input formats:
...
The unified agent API is fully backward compatible with existing agents. Agents created using the traditional workflow continue to function normally. You can use both workflows in the same cluster.

However, agents created with the unified agent API cannot be updated to use the traditional workflow parameters.
{: .note}
