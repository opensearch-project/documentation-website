...
[`model_config`](#the-model_config-object) | string | The model's configuration, including the `model_type`, `embedding_dimension`, and `framework_type`. |
...
| Field                 | Data Type         | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| :-------------------- | :---------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `model_type`          | string            | model type like `bert`, If you are using Huggingface model, you can find model type in config.json, refer to this [Huggingface model](https://huggingface.co/sentence-transformers/all-MiniLM-L6-v2/blob/main/config.json#L15)                                                                                                                                                                                                                                      |
| `embedding_dimension` | integer           | The dimension of model generated dense vector. If you are using Huggingface model, you can find on model card. For example this [Huggingface model](https://huggingface.co/sentence-transformers/all-MiniLM-L6-v2) `384 dimensional dense vector space`, 384 is the embedding dimension.                                                                                                                                                                            |
| `framework_type`      | string            | The framework model is using. For now, we support `sentence_transformers` or `huggingface_transformers`. For `sentence_transformers` model, it will output text embedding directly, so ml-commons won't do post processing. For `huggingface_transformers`, ml-commons will do post processing by applying mean pooling to get text embedding. Read this [sample huggingface model](https://huggingface.co/sentence-transformers/all-MiniLM-L6-v2) for more details |
| `all_config`          | string (optional) | This fied is just for reference purpose. You can put all model configuration to this field. For example, if you are using Huggingface model, you can minify the config.json file to one line and save all the content in `all_config` field. Once model uploaded, you can use get model API to get all model configurations stored in this field.                                                                                                                   |
