...
`parameters.message_history_limit` | Integer | Optional | `plan_execute_and_reflect` | The number of recent messages from conversation memory to include as context for the planner. Default is `10`. **Introduced 3.2**
`parameters.executor_message_history_limit` | Integer | Optional | `plan_execute_and_reflect` | The number of recent messages from conversation memory to include as context for the executor. Default is `10`. **Introduced 3.2**
...
`inject_datetime` | Boolean | Optional | `conversational`, `plan_execute_and_reflect` | Whether to automatically inject the current date into system prompt. Default is `false`. **Introduced 3.2**
`datetime_format` | String | Optional | `conversational`, `plan_execute_and_reflect` | Format string for date when `inject_datetime` is enabled. Default is `"yyyy-MM-dd'T'HH:mm:ss'Z'"` (ISO format). **Introduced 3.2**
