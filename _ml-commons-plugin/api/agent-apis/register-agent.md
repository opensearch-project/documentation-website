...
| `parameters.mcp_connectors` | Array | Optional | Array of Model Context Protocol (MCP) connector configurations that extend agent capabilities. For configuration details, see [Connecting to an external MCP server]({{site.url}}{{site.baseurl}}/ml-commons-plugin/agents-tools/mcp/mcp-connector/#step-3-register-an-agent-for-accessing-mcp-tools). |
| `parameters.mcp_connectors[].mcp_connector_id` | String | Required | The ID of a registered MCP connector. |
| `parameters.mcp_connectors[].tool_filters` | Array | Optional | Java-style regular expressions that select which MCP tools are available to the agent. |
| `parameters.mcp_connectors[].tool_descriptions` | Array | Optional | Objects that override MCP tool descriptions presented to the LLM. Available in OpenSearch 3.7 and later. |
