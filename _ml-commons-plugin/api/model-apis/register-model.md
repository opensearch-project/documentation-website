...
Specifically, for your convenience, we have provided a set of predefined model interfaces that can be generated automatically during model registration if you choose to follow the [connector blueprints]({{site.url}}{{site.baseurl}}/ml-commons-plugin/remote-models/blueprints/). 
The predefined model interface is generated based on the connector blueprint and the model's metadata, so you must strictly follow the blueprint when creating the connector in order to avoid exceptions.
