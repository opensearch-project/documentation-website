...
| `client_config`         | JSON object | No          | The client configuration object, which provides settings that control the behavior of the client connections utilized by the connector. These settings allow you to manage connection limits and timeouts, ensuring efficient and reliable communication.                                                                                                                            |
...
| `connection_timeout` | Integer   | The maximum amount of time (in seconds) the client will wait while trying to establish a connection to the server. A timeout prevents the client from waiting indefinitely and allows it to recover from unreachable network endpoints. |
| `read_timeout`       | Integer   | The maximum amount of time (in seconds) the client will wait for a response from the server after sending a request. Useful when the server is slow to respond or encounters issues while processing a request.                         |
