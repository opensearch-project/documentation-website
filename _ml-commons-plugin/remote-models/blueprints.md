...
| `skip_ssl_verification` | Boolean   | When set to `true`, SSL certificate verification is disabled for the connector, allowing connections to endpoints with self-signed or invalid certificates. This should only be used in development or testing environments. When set to `false` or omitted, SSL certificate verification is enabled (recommended for production). Default is `false`. |
