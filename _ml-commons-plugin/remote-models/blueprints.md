...
| `connector_id`                                   | String | No          | A custom ID for the connector. If omitted, OpenSearch auto-generates a connector ID. See [Custom connector IDs]({{site.url}}{{site.baseurl}}/ml-commons-plugin/api/connector-apis/create-connector/#custom-connector-ids). **Introduced 3.9** |
