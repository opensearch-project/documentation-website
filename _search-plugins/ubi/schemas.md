...
Below are the defined fields within the `ubi_events` index:
...
- `event_attributes`: An extensible structure that describes any important context about the event. This structure consists of two primary structures: `position` and `object`. The structure is extensible so you can add custom information about the event, such as the event's timing, user, or session.
