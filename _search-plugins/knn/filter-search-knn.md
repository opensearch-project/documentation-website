...
The response contains documents with the matching hotels:
...
The OpenSearch k-NN plugin version 2.2 introduced support for running k-NN searches with the Lucene engine using HNSW graphs. Starting with version 2.4, which is based on Lucene version 9.4, you can use Lucene filters for k-NN searches.
...
The following flow chart summarizes the Lucene algorithm.
...
The following request creates a new index called `hotels-index` with the `knn-filter` field called `location`:
...
The response returns three hotels that are nearest to the search point and have met the filter criteria:
