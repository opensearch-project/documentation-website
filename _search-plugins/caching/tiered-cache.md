...
OpenSearch 2.14 provides an implementation of _tiered spillover cache_. This implementation spills the evicted items from upper to lower tiers. The upper tier, such as the on-heap tier, is smaller in size but offers better latency. The lower tier, such as a disk cache, is larger in size but is slower in terms of latency. OpenSearch 2.14 offers on-heap and disk tiers. 
...
A tiered cache will fail to initialize if the `cache-ehcache` plugin is not installed or disk cache properties are not set. This is a fatal error and the node will fail to boot up.
{: .warning}

## Tiered cache settings

In OpenSearch 2.14, a request cache can use a tiered cache. To begin, configure the following settings in the `opensearch.yml` file.
...
`indices.request.cache.opensearch_onheap.expire` | Time unit | `MAX_VALUE` (disabled) | Specify a time-to-live (TTL) for the cached results. Optional.
...
`indices.request.cache.ehcache_disk.expire_after_access` | Time unit | `MAX_VALUE` (disabled) | Specify a time-to-live (TTL) for the cached results. Optional.
...
### Additional settings for `tiered_spillover` store.
Setting | Data type | Default | Description
:--- | :--- | :--- | :---
