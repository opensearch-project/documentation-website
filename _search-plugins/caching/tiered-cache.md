...
Setting | Data type | Default | Description
:--- | :--- | :--- | :---
`indices.requests.cache.tiered_spillover.policies.took_time.threshold` | Time unit | `0ms` | A policy used to determine whether to cache a query into the cache based on its took time. This is a dynamic setting. Optional.
`indices.requests.cache.tiered_spillover.disk.store.policies.took_time.threshold` | Time unit | `10ms` | A policy used to determine whether to cache a query into the disk tier of the cache based on its took time. This is a dynamic setting. Optional.
