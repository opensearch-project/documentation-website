...
| `knn_default_excludes` | `knn_default_excludes_factory` | Runs before any user-defined request processors. | Triggered for a search request unless the request sets `_source` to `true` or `false` or sets `stored_fields` to `_none_`. The processor has an effect only on indexes that contain `knn_vector` fields with `_source` enabled. See [Automatically exclude vectors from search results]({{site.url}}{{site.baseurl}}/vector-search/performance-tuning-search/#automatically-exclude-vectors-from-search-results). | Adds all `knn_vector` fields to `_source.excludes` so that vectors are omitted from the search response by default. Fields already covered by the request's `_source.includes` or `_source.excludes` are left unchanged. |
