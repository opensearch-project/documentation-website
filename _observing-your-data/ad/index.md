...
7. Specify a custom results index.
   ...
   - There are a few nested fields in the custom result index mapping, making it challenging to aggregate and visualize the data effectively. To address this, we offer an option **Enable flattened custom result index** to flatten the custom result index.
     - If you select this option for a custom results index, the plugin creates a separate index prefixed with the custom result index name and detector name. For example, if the detector `Test` uses the custom result index `abc`, a separate index with the alias `opensearch-ad-plugin-result-abc-flattened-test` will be created to store the anomaly detection results with nested fields flattened. In addition to creating a separate index, the plugin also sets up an ingest pipeline with a script processor. This pipeline is bound to the separate index and uses a Painless script to flatten all nested fields in the custom results index.
     - If you deselect this option for a detector that was previously enabled, the ingest pipeline set up for flattening will be deleted, and the separate result index will no longer reference this ingest pipeline as its default pipeline.
     - To use flattened custom result index, consider the following:
       - The flattened custom result index name is constructed based on the custom result index and detector name, and since the detector name is editable, conflicts can occur if the same name is reused.
