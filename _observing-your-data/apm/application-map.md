...
The application map provides an interactive topology visualization that is auto-generated from trace data by the Data Prepper `otel_apm_service_map` processor. The map displays your services as nodes with directional edges showing communication patterns, overlaid with RED metrics (Rate, Errors, Duration) for each service.

## Accessing the application map

To access the application map, navigate to your Observability workspace and select **APM** > **Application map** from the left navigation menu.
...
These metrics are generated by the `otel_apm_service_map` processor in Data Prepper and stored in Prometheus via remote write. For more information, see [APM service map processor]({{site.url}}{{site.baseurl}}/data-prepper/pipelines/configuration/processors/otel-apm-service-map/).

## Topology visualization

The application map auto-generates a directed graph from your trace data:
...
Time-series charts showing the trend of each metric over the selected time range:

- **Requests**: A chart showing request volume over time.
- **Latency**: A chart showing P50, P90, and P99 latency over time.
- **Faults (5xx)**: A chart showing server fault count over time.
- **Errors (4xx)**: A chart showing client error count over time.
...
- **Fault rate**: Show only services with fault rates (5xx errors) above a specified threshold.
- **Error rate**: Show only services with error rates (4xx errors) above a specified threshold.
- **Environment**: Filter services by deployment environment (for example, `production`, `staging`).
