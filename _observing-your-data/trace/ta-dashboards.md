...
OpenSearch 3.2 introduces user-configurable limits for Service map rendering. Previously, these limits were fixed constants, which could result in incomplete maps for environments with many services or dense interconnections.

In OpenSearch 3.1 and earlier, fixed limits (500 nodes and 1000 edges) could produce incomplete visualizations. With configurable limits in 3.2, you can increase the limits to render large topologies more completely, or lower them to improve client-side performance when focusing on smaller views.
...
- `observability:traceAnalyticsServiceMapMaxNodes`: Maximum number of service nodes displayed. **Default:** 500  
- `observability:traceAnalyticsServiceMapMaxEdges`: Maximum number of edges (service-to-service connections) displayed. **Default:** 1000

To configure these settings in OpenSearch Dashboards, go to **Management** > **Advanced Settings** and locate the **Trace Analytics** section. Adjust the values according to your environment and performance requirements, then save your changes. Higher values can increase browser memory and CPU usage.

The following image shows these settings in **Advanced Settings**:

![Service map configuration settings]({{site.url}}{{site.baseurl}}/images/ta-application-setting-services.png)
