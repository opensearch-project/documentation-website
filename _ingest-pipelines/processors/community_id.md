...
The `community_id` processor is used to generate community ID flow hash for the network flow tuples, the community ID flow hash algorithm is defined in [community-id-spec](https://github.com/corelight/community-id-spec). The hash value generated by this processor can be used to correlate all related network events, so that you can filter the network flow data by the hash value, or generate statistics by aggregating on the hash field. This processor supports TCP, UDP, SCTP, ICMP and IPv6-ICMP network protocols.

SHA-1 hash algorithm is used to generate the hash value.

The following is the syntax for the `community_id` processor:
...
|-----------|-----------|-----------|
`source_ip_field`  | Required  | The name of the field which contains the source IP address.  |
...
The following query creates a pipeline, named `commnity_id_pipeline`, that uses the `commnity_id` processor to generate hash value for the network flow tuple: 
