...
| `algorithm.<name>`          | String	   | Optional	 | The name of the chunking algorithm. Valid values are [`fixed_token_length`](#fixed-token-length-algorithm), [`delimiter`](#delimiter-algorithm) or [`fixed_char_length`](#fixed-char-length-algorithm). Default is `fixed_token_length`.	 |
