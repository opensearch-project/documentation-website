...
The `pipeline` processor allows a pipeline to reference and include another pre-defined pipeline. This can be useful when you have a set of common processors that need to be shared across multiple pipelines. Instead of redefining those common processors in each pipeline, you can create a separate base pipeline containing the shared processors, and then reference that base pipeline from other pipelines using the pipeline processor.
