...
`rename_alias_pattern` | If you want to rename aliases as you restore them, use this option to specify a regular expression that matches all aliases you want to restore. Use capture groups (`()`) to reuse portions of the alias name.
`rename_alias_replacement` | If you want to rename aliases as you restore them, use this option to specify the replacement pattern. Use `$0` to include the entire matching alias name, `$1` to include the content of the first capture group, and so on.
...
One way to avoid indexes naming conflicts when restoring indexes is to use the `rename_pattern` and `rename_replacement` options. You can then, if necessary, use the `_reindex` API to combine the two. Simlarly, to avoid aliases naming conflicts when restoring index with aliases, you can use the `rename_alias_pattern` and `rename_alias_replacement` options. The simpler way is to delete existing indexes prior to restoring from a snapshot. 
