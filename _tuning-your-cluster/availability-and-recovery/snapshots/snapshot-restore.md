...
`rename_pattern` | If you want to rename indexes , use this option to specify a regular expression that matches all the indexes you want to restore that you want to rename. Use capture groups (`()`) to reuse portions of the index name.
...
`rename_alias_pattern` | If you want to rename aliases, use this option to specify a regular expression that matches all the aliases you want to restore that you want to rename. Use capture groups (`()`) to reuse portions of the alias name.
...
One way to avoid index naming conflicts when restoring indexes is to use the `rename_pattern` and `rename_replacement` options. You can then, if necessary, use the `_reindex` API to combine the two. However, if might be simpler to delete the existing indexes that caused the conflict prior to restoring from a snapshot.
