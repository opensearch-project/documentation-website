...
The [start replication]({{site.url}}{{site.baseurl}}/tuning-your-cluster/replication-plugin/getting-started/#start-replication) and [create replication rule]({{site.url}}{{site.baseurl}}/tuning-your-cluster/replication-plugin/auto-follow/) operations are special cases. They involve background processes on the leader and follower clusters that must be associated with roles. When you perform one of these actions, you must explicitly pass the `leader_cluster_role` and
