...
[cron expressions](https://docs.opensearch.org/latest/monitoring-plugins/alerting/cron/), or manually load data into Logstash by running the query once.
...
To reindex the data into an OpenSearch domain, add the destination domain configuration in the `output{}` section like shown [here](https://docs.opensearch.org/latest/tools/logstash/index/).
