...
PDF generation is integrated directly into the Jekyll build process using the `jekyll-pdf-generator` plugin. PDFs are automatically generated during `jekyll build` and placed in `_site/pdfs/`, making them available for deployment alongside the HTML site.

### Requirements

The PDF generator uses the `pdfkit` gem, which requires `wkhtmltopdf` to be installed on your system:
...
PDFs are automatically generated when you build the Jekyll site:
...
The following documentation sections are available as PDFs (as configured in `_pdf-generator/config.yml`):
