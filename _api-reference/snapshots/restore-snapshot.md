...
| rename_alias_pattern | String | The pattern to apply to restored aliases. Aliases matching the rename pattern will be renamed according to `rename_alias_replacement`. <br /><br />The rename pattern is applied as defined by the regular expression that supports referencing the original text. <br /> <br />If two or more aliases are renamed into the same name, they will be merged.|
| rename_alias_replacement | String | The rename replacement string for aliases. See `rename_alias_pattern` for more information.|
