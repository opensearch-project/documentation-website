...
| cluster.routing.allocation.balance.prefer_primary | Boolean | When set to `true`, OpenSearch attempts to evenly distribute the number of primary shards between the cluster nodes. Enabling this setting does not always guarantee equal number of primary shards on each node, especially during a failover.
