...
`node_level_query_fanout` | Boolean | Whether to use node-level query fanout for this search request, overriding the `search.node_level_query_fanout.enabled` cluster setting when provided. Node-level query fanout groups shard-level `query_then_fetch` query and `can_match` requests by target data node. Default is `false`. Example: `POST index1/_search?node_level_query_fanout=true`. |
