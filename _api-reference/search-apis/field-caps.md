...
| `fields.<field>.<type>.searchable` | Boolean | Whether the field is indexed and searchable. On indexes that use a pluggable data format, numeric, `date`, `date_nanos`, `ip`, and `boolean` fields are reported as `false` because they are not indexed, although `range`, `term`, and `terms` queries on them are still served from doc values. For more information, see [Pluggable data format indexes]({{site.url}}{{site.baseurl}}/mappings/mapping-parameters/index-parameter/#pluggable-data-format-indexes). |
