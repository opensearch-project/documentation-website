...
index.codec | The compression type to use to compress stored data. Available values are `best_compression` (optimizes for retrieval speed) and `default` (optimizes for better compression at the expense of speed, leading to smaller data sizes on disk).
