...
index.codec | The compression type to use to compress stored data. Available values are `default`, which optimizes for retrieval speed, and `best_compression`, which optimizes for better compression at the expense of speed, leading to smaller data sizes on disk. Alternatively, you can use `zstd` and `zstd_no_dict` to optimize for better compression and retrieval speed. `zstd` creates a dictionary with the data being indexed, uses that for compression, and has a minor trade-off on indexing speed. `zstd_no_dict` does not create a dictionary, leading to faster indexing speeds.
