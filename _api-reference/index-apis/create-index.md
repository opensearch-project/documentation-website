...
index.codec | The compression type to use to compress stored data. Available values are `default`, which optimizes for retrieval speed, `best_compression`, which optimizes for better compression at the expense of speed, leading to smaller data sizes on disk. `zstd` and `zstd_no_dict` optimizes for better compression alongside retrieval speed. `zstd` creates a dictionary with the data being indexed and uses it for compression with a minor trade off on indexing speed. `zstd_no_dict` does not create a dictionary, leading to faster indexing speeds.
