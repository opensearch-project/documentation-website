...
| `primary_only` | Boolean | If `true`, the merge operation only performs on the primary shards, this is useful when you want to take snapshot for the specified indices(snapshot only copies segements from the primary shards) after the force merge operation completes, performing the force merge operation only on primary shards can reduce the resouces consumption. Default is `false`. |
