...
OpenSearch ingest APIs simplify the data ingestion process with a standard and structured way to process input data, and ingest pipelines pre-process and transform data before it's indexed in OpenSearch. A pipeline consists of processors, customizable tasks that run in the order they are defined in the pipeline. The transformed data is ingested in your index after each processor completes.
