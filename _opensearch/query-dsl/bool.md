...
`should` | Logical `or` operator. The results must match at least one of the queries, but optionally, it can match more than one query. Each matching `should` clause increases the relevancy score.  You can set the minimum number of queries that must match using the `minimum_number_should_match` parameter.
