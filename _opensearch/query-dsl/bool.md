...
`should` | Logical `or` operator. The results must match at least one of the queries, but optionally, it can match more than one query. Each matching `should` clause increases the relevancy score.  
`minimum_number_should_match` | Optional parameter for use with a `should` query clause. You can set the minimum number of queries that must match using the `minimum_number_should_match` parameter. The default value is 1. If `minimum_number_should_match` is set to 2, then the results must match two queries.
