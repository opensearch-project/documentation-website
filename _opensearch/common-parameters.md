...
Stack traces | To include the error stack trace in the response when an exception is raised, add `error_trace=true` to the request URL. <br> <br> When the config setting `ttp.detailed_errors.enabled: false`, the request of `error_trace=true` will not contain the detailed error message.| `GET <index_name>/_search?error_trace=true`
