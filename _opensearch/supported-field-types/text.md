...
A text field type is a string that is analyzed. It is used for full-text search since it allows partial matches. Searches with multiple terms can match some but not all of them. Depending on the analyzer, results can be case-insensitive, stemmed, stopwords removed, synonyms applied, etc.


If you need to use a field for exact-value search, map it as [`keyword`]({{site.url}}{{site.baseurl}}/opensearch/supported-field-types/keyword/) instead.
{: .note }
...
Create a mapping with a text field which stores character offsets in a term vector:
