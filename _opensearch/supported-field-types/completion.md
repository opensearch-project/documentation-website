...
To query completion field types, specify the prefix that you want to search for and the name of the field where to look for suggestions.
...
Parameter | Description 
:--- | :--- 
`fuzziness` | Fuzziness can be set as: <br> 1. An integer that specifies the maximum allowed [Levenshtein distance](https://en.wikipedia.org/wiki/Levenshtein_distance) for this edit. <br> 2. `AUTO`: strings of 0–2 characters must match exactly, strings of 3–5 characters allow one edit, and strings longer than 5 characters allow two edits.<br> Default is `AUTO`.
...
`prefix_length` | An integer that specifies the minimum length of the matched prefix to start returning suggestions. If the prefix of `prefix_length` is not matched, but the search term is still within Levenshtein distance, no suggestions are returned. Default is 1.
