...
`synonym_analyzer` | Optional | String | Specifies the name of an analyzer to use when parsing the synonym rules, instead of the analysis chain in which this filter is defined. This can be any analyzer available to the index, including built-in analyzers (such as `standard`, `simple`, `stop`, `whitespace`, or `keyword`), language analyzers, analyzers registered by plugins, and custom analyzers defined in the same index. If the analyzer cannot be resolved, the analysis chain is used and no error is returned. By default, the analysis chain is used.
