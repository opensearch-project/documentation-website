...
**OpenSearch sink** | `bulk_size` (MiB) | Determines the size of bulk requests; smaller bulks flush sooner. | 1--5 MiB | Very small `bulk_size` increases the number of bulk requests, adds HTTP/TLS overhead, causes more thread pool contention, produces smaller Lucene batches, and lowers throughput. Very large `bulk_size` increases the time needed to fill a batch and can cause memory spikes, larger retries, and higher p95/p99 latency.
