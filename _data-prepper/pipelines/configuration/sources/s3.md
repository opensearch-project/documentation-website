...
At ingestion time, data received by the S3 source can be parsed by codecs. However, codecs only support a particular type of data input type, compressions types, and processors. The following table describes each codec, data type, and processor combination.
