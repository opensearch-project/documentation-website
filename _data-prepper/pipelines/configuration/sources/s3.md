...
To use Amazon SQS notifications, you configure [S3 Event Notifications](https://docs.aws.amazon.com/AmazonS3/latest/userguide/NotificationHowTo.html) on your S3 bucket. After Amazon SQS is configured, the `s3` source receives messages from Amazon SQS. When the SQS message indicates that an S3 object was created, the `s3` source loads the S3 objects and then parses them using the configured [codec](#codec). You can also configure the `s3` source to use [Amazon S3 Select](https://docs.aws.amazon.com/AmazonS3/latest/userguide/selecting-content-from-objects.html) instead of Data Prepper to parse S3 objects.
...
`default_bucket_owner` | No | String | The AWS account Id for the owner of S3 buckets. See [Cross-account S3 access](#s3_bucket_ownership) for more details.
`bucket_owners` | No | Map  | A map of bucket names to owning account Ids. See [Cross-account S3 access](#s3_bucket_ownership) for more details.
...
### scan bucket<a name="scan_bucket"></a>
...
* `s3ObjectsEmpty` -- The number of S3 objects that are empty by having a size of `0`. These will be skipped.
...
* `sqsVisibilityTimeoutChangedCount` -- The number of times that the `s3` source has changed the visibility timeout for an SQS message. This includes multiple visibility timeout changes on the same message.
...
* `acknowledgementSetCallbackCounter` -- The number of times that the `s3` source receives an acknowledgment from Data Prepper acknowledgments.
