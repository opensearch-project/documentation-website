...
`s3` is a source plugin that reads events from [Amazon Simple Storage Service](https://aws.amazon.com/s3/) (Amazon S3) objects. The following table describes options you can use to configure the `s3` source.

This source requires an [Amazon Simple Queue Service (SQS)](https://aws.amazon.com/sqs/) (Amazon SQS) queue which receives
[S3 Event Notifications](https://docs.aws.amazon.com/AmazonS3/latest/userguide/NotificationHowTo.html).
The S3 Source will receive messages from the configured SQS queue.
For each SQS message indicating that an S3 object was created, the `s3` source will load those created S3 objects.
Then the `s3` source will parse the S3 object using the configured [codec](#codec).
Optionally, you can configure the `s3` source use 
[Amazon S3 Select](https://docs.aws.amazon.com/AmazonS3/latest/userguide/selecting-content-from-objects.html)
to parse the file rather than have Data Prepper process it.


### Example: Un-Compressed Logs

The following configuration shows a minimum configuration for reading newline-delimited logs which are not compressed.
...
In order to use the `s3` source, you must configure your AWS permissions to grant Data Prepper access.
You will use a configuration similar to the following.
...
compression | No                                | String                  | The compression algorithm to apply: `none`, `gzip`, or `automatic`. If set to `automatic`, then S3 objects with keys ending in `.gz` will be decompressed using gzip. Default value is `none`.
codec | Yes, unless s3_select is provided | [codec](#codec)         | The codec to apply. See [codec](#codec) for available codecs and how to configure them. 
s3_select | No, unless not codec is provided  | [s3_select](#s3_select) | The S3 Select configuration to extract events from S3 objects. See [s3_select](#s3_select) for details. 
sqs | Yes                               | [sqs](#sqs)             | The Amazon SQS configuration. See [sqs](#sqs) for details.
aws | Yes                               | [aws](#aws)             | The AWS configuration. See [aws](#aws) for details.
...
acknowledgements | No                                | Boolean                 | If true, enables end-to-end acknowledgements for the S3 source. Defaults to `false`.
...
The `codec` configuration determines how the `s3` source will parse each S3 object.

#### newline

Parses files where each single line is a single log event. 
This is the ideal codec to use for most application logs since each event gets a single line.
...
This codec is also suitable for S3 Objects which have individual JSON objects on each line. 
You can use this codec and a 
[`parse_json`]({{site.url}}{{site.baseurl}}/data-prepper/pipelines/configuration/processors/parse-json/)
processor to parse each line.

#### json

Parses each S3 Object as a single JSON object. 
This codec will parse each JSON object from a JSON array. 
Then the codec creates a Data Prepper log event for each object in the JSON array.

#### csv

Parses S3 objects as a comma-separated values (CSV) format.
Each line of data will produce a Data Prepper log event.

Option | Required | Type        | Description
:--- |:---------|:------------| :---
delimiter | String   | Integer     | The delimiter separating columns. Defaults to `,`.
quote_character | String   | String      | The character used as a text qualifier for CSV data. Defaults to double quote (`"`).
...
Option | Required               | Type        | Description
:--- |:-----------------------|:------------| :---
expression | When using `s3_select` | String      | The expression that is used to query the object. Maps directly to the [Expression](https://docs.aws.amazon.com/AmazonS3/latest/API/API_SelectObjectContent.html#AmazonS3-SelectObjectContent-request-Expression) property.
...
input_serialization | When using `s3_select` | String      | Provide the S3 Select file format Amazon S3 uses this format to parse object data into records and returns only records that match the specified SQL expression. Maybe be `csv`, `json`, or `parquet`.
...
csv | No                     | [csv](#s3_select_csv) | Provide the CSV configuration for processing CSV data.
json | No                     | [json](#s3_select_json) | Provide the JSON configuration for processing JSON data.

#### csv<a name="s3_select_csv"></a>

You can use these options to describe how the CSV file is formatted. 
These fields map directly to options available in S3 Select's [CSVInput](https://docs.aws.amazon.com/AmazonS3/latest/API/API_CSVInput.html) data type.
...
You can use these options to configure JSON processing with S3 Select.
These fields map directly to options available in S3 Select's [JSONInput](https://docs.aws.amazon.com/AmazonS3/latest/API/API_JSONInput.html) data type.
