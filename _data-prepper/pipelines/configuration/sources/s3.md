...
Parameter | Required | Type | Description
:--- | :--- | :--- | :---
`object_name` | Yes | String | The file name of the companion object (for example, `_SUCCESS` or `manifest.json`) that resides in the same S3 directory as the data object. This file is downloaded and evaluated against the `when` expression.
`when` | Yes | String | A Data Prepper expression evaluated against the content of the companion object. The S3 data object is processed only when this expression evaluates to `true`. For example, `/status == "complete"`.
`applicable_prefix` | No | List | A list of S3 key prefix strings. The condition is only evaluated for objects whose key starts with at least one of these prefixes. When omitted, the condition applies to all objects in the bucket.
`retry_delay` | No | String | How long to wait before rechecking the condition after it is not met. Supports ISO 8601 duration notation, for example, `PT5M`. Minimum is `PT1M`, maximum is `PT60M`. Default is `PT5M`.
`max_retry` | No | Integer | The maximum number of times to recheck the condition before processing the object regardless. Accepts values from `0` to `100`. Default is `10`. When `max_retry` is reached, the object is processed as-is.
