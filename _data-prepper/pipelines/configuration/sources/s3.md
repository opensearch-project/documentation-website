...
### Bucket ownership<a name="s3_bucket_ownership"></a>

When Data Prepper gets data from the S3 bucket, it verifies the ownership of the bucket using the 
[bucket owner condition](https://docs.aws.amazon.com/AmazonS3/latest/userguide/bucket-owner-condition.html).
By default, Data Prepper will expect that the S3 bucket is owned by the same account that owns the SQS queue.
If not SQS queue is provided, it will use the role ARN in the `aws` configuration.

If you are using cross-account S3 access, you need to configure how Data Prepper checks this condition.

If all of your buckets are in one other account, you can specify the `default_bucket_owner` and set it to account Id of that account owning the other buckets.
If your buckets are in multiple accounts, you should use the `bucket_owners` map.

In this example, the SQS queue is owned by account `000000000000`. It will have data from two S3 buckets: `my-bucket-01` and `my-bucket-02`.
These buckets are owned by different accounts - `my-bucket-01` is owned by `123456789012` and `my-bucket-02` is owned by `999999999999`.
You can configure Data Prepper using the following.
...
You can use both the `bucket_owners` and `default_bucket_owner` together.
...
bucket_owners | No | Map      | A map of S3 bucket names to AWS account Ids. When provided, the `s3` source will validate the bucket is owned by the account. This allows for using buckets in multiple accounts.
disable_bucket_ownership_validation | No | Boolean  | If `true`, the S3Source will not attempt to validate that the bucket is owned by the expected account. By default, this is the same account that owns the Amazon SQS queue, but see [bucket ownership](#s3_bucket_ownership) for more details. Defaults to `false`.
