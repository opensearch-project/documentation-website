...
* `s3ObjectsNotFound`: The number of S3 objects that the `s3` source failed to read due to an AMazon S3 "Not Found" error. These are also counted toward `s3ObjectsFailed`.
