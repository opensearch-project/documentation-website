...
`s3` is a source plugin that reads events from [Amazon Simple Storage Service (Amazon S3)](https://aws.amazon.com/s3/) objects.
You can configure the source to either use an [Amazon Simple Queue Service (Amazon SQS)](https://aws.amazon.com/sqs/) queue or scan an S3 bucket:

- To use Amazon SQS notifications,  configure [S3 event notifications](https://docs.aws.amazon.com/AmazonS3/latest/userguide/NotificationHowTo.html) on your S3 bucket. After Amazon SQS is configured, the `s3` source receives messages from Amazon SQS. When the SQS message indicates that an S3 object was created, the `s3` source loads the S3 objects and then parses them using the configured [codec](#codec). 
- To use an S3 bucket, configure the `s3` source to use [Amazon S3 Select](https://docs.aws.amazon.com/AmazonS3/latest/userguide/selecting-content-from-objects.html) instead of Data Prepper to parse S3 objects.
...
`buffer_timeout` | No | Duration  | The amount of time allowed for writing events to the Data Prepper buffer before timeout occurs. Any events that the Amazon S3 source cannot write to the buffer during the amount of time set are discarded. Default is `10s`.
...
`default_bucket_owner` | No | String | The AWS account ID for the owner of S3 buckets. For more information, see [Cross-account S3 access](#s3_bucket_ownership)
`bucket_owners` | No | Map  | A map of bucket names that includes the IDs of the accounts that own that buckets. For more information, see [Cross-account S3 access](#s3_bucket_ownership) for more details.
...
`sts_external_id` | No | String | An STS external ID to use when Data Prepper assumes the STS role. For more information, see the documentation for `ExternalID` in the [STS AssumeRole](https://docs.aws.amazon.com/STS/latest/APIReference/API_AssumeRole.html) reference.
...
* `s3ObjectsEmpty`: The number of S3 objects that are considered empty since they have a size of `0`. These objects will be skipped by the `s3` source.
...
* `acknowledgementSetCallbackCounter`: The number of times that the `s3` source receives an acknowledgment from Data Prepper.
