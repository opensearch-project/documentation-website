...
If your S3 objects or Amazon SQS queues do not use the [AWS Key Management Service (AWS KMS](https://aws.amazon.com/kms/), remove the `kms:Decrypt` permission.
...
* `sqsMessageDelay`: Measures the amount of time from when S3 creates an object to when it is fully parsed.
