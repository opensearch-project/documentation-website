...
## Codec and processor combinations 

At ingestion time, data received by the S3 source can be parsed by codecs. However, codecs only support a particular type of data input type, compressions types, and processors. The following table describes each codec, data type, and processor combination where performance of the pipeline can be improved. 

| **Input Data Type** | **Compression type** | **Codec** | **Processor** | Notes |
|---|---|---|---|---|
| JSON Array | None /gzip/ Automatic | JSON |  JSON |  |
| ND JSON | None /gzip/ Automatic | Newline | JSON |  |
| CSV | None /gzip/ Automatic | CSV | None | Header is automatically detected and used for index mapping with the CSV codec. |
| CSV | None /gzip/ Automatic | Newline | CSV | Header is detected if `header_destination` is configured in the `newline` codec. <br>The header can be used in the CSV processor under `column_names_source_key`. |
| CSV | None /gzip/ Automatic | None | CSV |  |
| Parquet | None /gzip/ Automatic | Parquet | None |  |
| Parquet | Snappy / Automatic | S3 Select | None |  |
| Avro | None /gzip/ Automatic | Avro | None |  |
| Avro | Snappy / Automatic | Avro | None |  |
