...
`workers` | No | Integer | Default value 1. Min is 1 and Max is 1000. When set to value `n` which more than 1, `n` number of worker threads are created. Each of the worker threads subscribe to listening to SQS messages and upon receiving a message, process it independent of the other workers.
