...
- To use an S3 bucket, configure the `s3` source to use Amazon S3 Select instead of OpenSearch Data Prepper to parse S3 objects.
...
`visibility_timeout` | No | Duration | The visibility timeout to apply to messages read from the Amazon SQS queue. This parameter should be set to the amount of time that Data Prepper may take to read all the S3 objects in a batch. If processing exceeds this timeout and `visibility_duplication_protection` is not enabled, the message becomes visible in the queue again and may be reprocessed, which can create duplicate data in the sink. The maximum value is 12 hours (`43200s`). Default is `30s`.
...
`visibility_duplication_protection` | No | Boolean | If set to `true`, Data Prepper attempts to avoid duplicate processing by extending the visibility timeout of SQS messages. Until the data reaches the sink, Data Prepper regularly calls `ChangeMessageVisibility` to avoid rereading the S3 object. This option takes effect only when `acknowledgments` is enabled on the `s3` source, because the visibility timeout is extended through end-to-end acknowledgments. When `acknowledgments` is not enabled, the visibility timeout is not extended even if this option is set to `true`. To use this feature, you must grant permissions to `sqs:ChangeMessageVisibility` on the IAM role. Default is `false`.
`visibility_duplicate_protection_timeout` | No | Duration | Sets the maximum total length of time that a message will not be processed when using `visibility_duplication_protection`. The minimum value is `30s` and the maximum value is 24 hours (`86400s`). Default is 2 hours (`7200s`).
`max_receive_attempts` | No | Integer | The maximum number of times a message can be received from the queue before it is automatically deleted. When a message has been received more than this number of times, Data Prepper deletes it from the queue. This helps prevent repeatedly failing messages from blocking the queue. Minimum value is `1`. When this option is configured, the `sqsMessageDelay` metric is not recorded for messages that have been received more than once. Disabled by default.
...
`processing_conditions` | No | List | A list of conditions that must be satisfied before an S3 object is processed. Each condition downloads a companion object located in the same S3 directory and evaluates a `when` expression against its content. If any condition is not met, processing of the S3 object is deferred and retried. Conditions are evaluated once per S3 object, not per record. See [processing_conditions](#processing_conditions).
...
Use the following parameters in each `processing_conditions` entry.

Parameter | Required | Type | Description
:--- | :--- | :--- | :---
`object_name` | Yes | String | The name of the companion object (for example, `_SUCCESS` or `manifest.json`) located in the same S3 directory as the data object. The companion object is downloaded and evaluated against the `when` expression.
`when` | Yes | String | A Data Prepper expression that is evaluated against the content of the companion object. The S3 data object is processed only when this expression evaluates to `true`. For example, `/status == "complete"`.
`applicable_prefix` | No | List | A list of S3 key prefix strings. The condition is evaluated only for objects whose keys start with at least one of these prefixes. When omitted, the condition applies to all objects in the bucket.
`retry_delay` | No | String | The duration to wait before rechecking the condition when it is not met. Supports ISO 8601 duration format (for example, `PT5M`). Minimum is `PT1M`, maximum is `PT60M`. Default is `PT5M`.
`max_retry` | No | Integer | The maximum number of times to recheck the condition before processing the object regardless of the result. Accepts values from `0` to `100`. Default is `10`. When this limit is reached, the object is processed as-is.
`codec` | No | Codec | A codec plugin used to parse the companion object. When configured, the `when` expression is evaluated against the first record parsed by the `codec`. When omitted, the companion object is parsed as a flat JSON document.

The following example configures `processing_conditions` at the bucket level:
