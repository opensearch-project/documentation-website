...
`s3` is a source plugin that reads events from [Amazon Simple Storage Service (S3)](https://aws.amazon.com/s3/) objects. It requires an [Amazon Simple Queue Service (SQS)](https://aws.amazon.com/sqs/) queue that receives [S3 Event Notifications](https://docs.aws.amazon.com/AmazonS3/latest/userguide/NotificationHowTo.html). After SQS is configured, the `s3` source receives messages from SQS. When the SWS message indicates an S3 object was created, the `s3` source loads the objects than parses them using the configured [codec](#codec). You can also configure the `s3` source to use [Amazon S3 Select](https://docs.aws.amazon.com/AmazonS3/latest/userguide/selecting-content-from-objects.html) to parse S3 objects instead of Data Prepper.

## AWS IAM Permissions

In order to use the `s3` source, configure your AWS permissions to grant Data Prepper access to S3. You can use a configuration similar to the following:
...
If your S3 objects or SQS queues do use the [AWS Key Management Service](https://aws.amazon.com/kms/), remove the `kms:Decrypt` permission.


## Configuration

The following table describes options you can use to configure the `s3` source.
