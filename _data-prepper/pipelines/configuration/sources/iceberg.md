...
The following example pipeline specifies an `iceberg` source. It reads incremental changes from an Iceberg table using a REST catalog with S3 storage. The `catalog` is defined at the top level so that all tables share the same catalog configuration.

In the sink configuration, `action` is set to `${getMetadata("bulk_action")}` so that INSERT and UPDATE operations are upserted while DELETE operations remove the corresponding document. The `document_id` is set to `${getMetadata("document_id")}`, which concatenates the `identifier_columns` values with a `|` separator so that each Iceberg row maps to a unique OpenSearch document.
...
When a table needs a different catalog, specify `catalog` at the table level to override the top-level definition:
...
:--- | :--- | :--- | :---
`catalog` | No | Map | Default catalog properties applied to all tables. When a table specifies its own `catalog`, it fully replaces this top-level definition. See [catalog](#catalog) for common properties.
...
`catalog` | No | Map | A map of Apache Iceberg catalog properties. When specified, fully replaces the top-level `catalog`. See [catalog](#catalog) for common properties.
...
`storage_path` | No | String | Local directory for shuffle intermediate files. Default is `${data-prepper.dir}/data/shuffle` when `data-prepper.dir` is set, otherwise `${java.io.tmpdir}/data-prepper-shuffle`.
...
* `iceberg_operation`: The CDC operation type. Set to `INSERT` or `DELETE`. When a row is updated in Iceberg, the source receives a DELETE and INSERT pair with the same `identifier_columns` values. The source recognizes this as an update and emits only the INSERT, since upserting by `document_id` produces the correct result without a separate delete.
