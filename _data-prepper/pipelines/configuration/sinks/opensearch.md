...
`index_type` | No | String | Tells the sink plugin what type of data it is handling. Valid values are `custom`, `trace-analytics-raw`, `trace-analytics-plain-raw`, `trace-analytics-service-map`, `log-analytics`, `log-analytics-plain` `metric-analytics`, `metric-analytics-plain` or `management-disabled`. When using `otel_logs_source` with `output_format: otel`, `index_type` should be set to `log-analytics-plain` for ss4o compliant data. Similarly for `otel_metrics_source` with `output_format: otel`, `index_type` should be set to `metric-analytics-plain` and for `otel_trace_source` with `output_format: otel`, `index_type` should be set to `trace-analytics-plain-raw`.  Default is `custom`.

trace-analytics-raw, trace-analytics-plain-raw, trace-analytics-service-map, log-analytics, log-analytics-plain, metric-analytics, metric-analytics-plain, custom, management_disabled
