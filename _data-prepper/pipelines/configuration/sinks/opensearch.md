...
enable_request_compression | No | Boolean | Whether to enable request compression when sending requests to OpenSearch. For `distribution_version` set to `es6`, default is `false`, otherwise default is `true`.  
