...
`threshold` | Yes | [Threshold](#threshold-configuration) | Sets configurations for when to write an object to S3. 
...
`maximum_size` | Yes | String | The maximum number of bytes that the S3 bucket can ingest. This applies after compression. Defaults to `50mb`.
...
`buffer_type` is an optional configuration that records stored events temporarily before flushing them into an S3 bucket. The default value is `in_memory`. Use of one of the following options: 
...
Option | Required | Type | Description
:--- | :--- | :--- | :---
`path_prefix` | Yes | String | The S3 key prefix path to use. Accepts date-time formatting. For example, you can use `%{yyyy}/%{MM}/%{dd}/%{HH}/` to make hourly "folders" in S3. By default, events write to the root of the bucket. 
...
The `avro` codec writes events as an [Apache Avro](https://avro.apache.org/) document.

Because Avro requires a schema, you may either define the schema yourself, or Data Prepper will auto-generate a schema.
In general, you should define your own schema as it will most accurately reflect your needs.
In cases where your data is uniform, you may be able to automatically generate a schema.
Auto-generated schemas are based on the first event received.
...
`auto_schema` | No | Boolean | Set to true to automatically generate the Avro [schema declaration](https://avro.apache.org/docs/current/specification/#schema-declaration) from the first event.
...
The `ndjson` codec writes each single line as a JSON object.
