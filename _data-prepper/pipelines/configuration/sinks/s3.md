...
The `bucket_owners` map specifies account IDs for buckets across accounts, for example, `my-bucket-01` owned by `123456789012` and `my-bucket-02` owned by `999999999999`, as shown in the following configuration:
...
`bucket_owners` and `default_bucket_owner` can be used together.
...
`bucket_owners` | No       | Map                                             | Map of bucket names and their account owner IDs for cross-account access. See [Cross-account S3 access](#s3_bucket_ownership). 
`default_bucket_owner` | No       | String                                          | AWS account ID for an S3 bucket owner. See [Cross-account S3 access](#s3_bucket_ownership). 
`codec` | Yes      | [Codec](#codec)                                 | Serializes data in S3 objects.
`aws` | Yes      | AWS                                             | AWS configuration. See [aws](#aws).
...
`object_key` | No       | [Object key](#object-key-configuration)         | Sets `path_prefix` and `file_pattern` for object storage. The file pattern is `events-%{yyyy-MM-dd'T'hh-mm-ss}`. By default, those objects are found inside the root directory of the bucket. The `path_prefix` is configurable.
`compression` | No       | String                                          | Compression algorithm (`none`, `gzip`, `snappy`). Default is `none`. 
...
Because Avro requires a schema, you may either define the schema yourself or have Data Prepper automatically generate it. It is recommended that you define your schema to accurately reflect your needs.

We recommend that you make your Avro fields use a null [union](https://avro.apache.org/docs/current/specification/#unions). Without the null union, each field must be present or the data will fail to write to the sink. If you can be certain that each each event has a given field, you can make it non-nullable.
