...
By default, the S3 sink does not require `bucket_owners`. If `bucket_owners` is configured and a bucket is not included in one of these mapped configurations, the `default_bucket_owner` defaults to the account id from the `aws.sts_role_arn`. 
