...
We recommend that you make your Avro fields use a null [union](https://avro.apache.org/docs/current/specification/#unions) unless you can be completely certain that the data will be present.
Any events which do not have values for types which do not have a null union will fail to write to the destination.

When you provide your own Avro schema, the schema you provided defines the final structure of the data.
And extra values in the incoming events will not be included in the final destination.
Also, to avoid confusion between your provided schema and the `include_keys` or `exclude_keys` sink configurations, these values are disallowed with a user-provided schema.
...
You may use the sink 'include_keys' and `exclude_keys` configurations to control what data is included in the auto-generated schema.
...
You must set the `buffer_type` to `in_memory` when using Parquet.
