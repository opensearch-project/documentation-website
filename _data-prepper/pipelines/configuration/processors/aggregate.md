...
local_mode | No | Boolean | when `local_mode` is set `true`, the aggregation is locally in each of the data-prepper nodes, instead of forwarding events to hash based on `identification_keys`. Default is `false`.
