...
| `delete_when` | No | Specifies under what conditions the deletion should be performed. (Example: <i>value= "/some_key == null"</i> - Deletion runs only if <i>some_key</i> is null or does not exist.) |
|`delete_from_element_when` | No | Specifies the condition needed to delete the key from each element of a list when using `iterate_on`                                                                              |
