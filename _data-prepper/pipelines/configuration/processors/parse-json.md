...
| `handle_failed_events` | No | Enum | Determines how to handle events with JSON processing errors. Options include `skip`, which will log the error and send the event downstream to the next processor, `skip_silently`, which will send the event downstream to the next processor without logging the error.|
