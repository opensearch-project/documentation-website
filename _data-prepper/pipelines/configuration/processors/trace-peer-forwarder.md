...
In the preceding pipeline, events are forwarded in the `otel-trace-pipeline` to the correct peer, and no forwarding is performed in `raw-pipeline` or `service-map-pipeline`.