...
| value_grouping | Specifies whether to group values using predefined value grouping delimiters - `{...}`, `[...]', `<...>`, `(...)`, `"..."`, `'...'`, `http://... (space)` and `https:// (space)`. If this flag is enabled, then content of value between the delimiters is considered as one entity and is not parsed for key values. Default is false.| If `value_grouping` is `true`, `{"key1=[a=b,c=d]&key2=value2"}` will parse into `{"key1": "[a=b,c=d]", "key2": "value2"}`. |
