...
destination | No | String | The key that is the destination for the parsed source output. Outputting the parsed source overwrites the value of the key if it already exists. Default value is `parsed_message`
...
non_match_value | No | String | When a key/value cannot be successfully split, the key/value is placed in the `key` field and the specified value in the value field. Default value is `null`.
