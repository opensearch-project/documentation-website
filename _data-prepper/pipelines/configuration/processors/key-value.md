...
| value_grouping | Specifies whether to group values using pre-defined value grouping delimiters - `{...}`, `[...]', `<...>`, `(...)`, `"..."`, `'...'`, `http://... (space)` and `https:// (space)`. If this flag is enabled, then content of value between the delimiters is considered as one entity and is not parsed for key values. Default is false.| If `value_grouping` is `true`, `{"key1=[a=b,c=d]&key2=value2"}` will parse into `{"key1": "[a=b,c=d]", "key2": "value2"}`. |
| drop_keys_with_no_value | Specified if key should be dropped if it has null as its value. Default is false. | If `drop_keys_with_no_value` is `true, `{"key1=value1&key2"}` will parse into `{"key1": "value1"}`. |
