...
The `encoder` setting is not supported for fields with the `half_float` data type. To quantize [`half_float` vectors]({{site.url}}{{site.baseurl}}/mappings/supported-field-types/knn-memory-optimized/#half-float-vectors), set the `compression_level` mapping parameter to `16x` (1-bit quantization), or use `1x` for unquantized 16-bit floating-point (FP16) storage.
{: .important}
