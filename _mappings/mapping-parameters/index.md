...
[`coerce`]({{site.url}}{{site.baseurl}}/mappings/mapping-parameters/coerce/) | Controls whether OpenSearch attempts to normalize and convert values to match the field's data type during indexing (for example, converting strings to numbers or truncating floats to integers).
...
[`disable_objects`]({{site.url}}{{site.baseurl}}/mappings/mapping-parameters/disable-objects/) | Controls whether dotted field names are expanded into nested object structures or treated as literal flat field identifiers, ensuring deterministic ingestion for metrics and analytics workloads.
...
[`fielddata`]({{site.url}}{{site.baseurl}}/mappings/mapping-parameters/field-data/) | Enables loading analyzed tokens from a `text` field into memory for sorting, aggregations, and scripting. Disabled by default because of high memory cost.
[`fields`]({{site.url}}{{site.baseurl}}/mappings/mapping-parameters/fields/) | Enables indexing the same field in multiple ways by defining additional subfields with alternate mappings (for example, different data types or analyzers) to support varied search and aggregation requirements.
...
[`ignore_malformed`]({{site.url}}{{site.baseurl}}/mappings/mapping-parameters/ignore-malformed/) | Instructs the indexing engine to ignore values that do not match the field's expected format, storing documents even if some fields contain data that cannot be parsed.
