...
The URL outputted takes users to a Dask Dashboard that visualizes the generation process. Users can keep track of CPU and memory of each worker as well as obtain a CPU flamegraph of the generation process. This is helpful for monitoring the load generation's resources when generating data or optimizing generation when using a custom python module.
