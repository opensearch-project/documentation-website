...
- The  [`index.neural_search.hybrid_collapse_docs_per_group_per_subquery`]({{site.url}}{{site.baseurl}}/vector-search/settings/#hybrid-collapse-docs-per-group) setting was introduced in OpenSearch 3.2 but is no longer needed and is being deprecated. Previously, the collector selected the top groups and then collected all documents within each group. Now it collects the top documents directly along with their associated groups, avoiding the collection of non-competitive documents. The setting is still accepted for backward compatibility but has no effect.
