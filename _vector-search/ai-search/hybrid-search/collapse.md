...
- The  [`index.neural_search.hybrid_collapse_docs_per_group_per_subquery`]({{site.url}}{{site.baseurl}}/vector-search/settings/#hybrid-collapse-docs-per-group) setting is deprecated and has no effect. In case, if this setting currently exists in the index, you can safely remove it. The search results are entirely controlled by the `size` parameter in the search request.
