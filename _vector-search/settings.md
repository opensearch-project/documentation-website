...
`knn.dynamic_mapping.enabled` | Dynamic | `false` | Enables [dynamic mapping]({{site.url}}{{site.baseurl}}/mappings/supported-field-types/knn-vector/#dynamic-mapping) of `knn_vector` fields, including auto-inference of unmapped flat numeric arrays and dynamic templates that reference `knn_vector` as `match_mapping_type`. Disabled by default because auto-inference is a shape-based heuristic and can misclassify non-vector numeric arrays.
