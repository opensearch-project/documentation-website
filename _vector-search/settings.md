...
| `knn.remote_index_build.size.max`       | Dynamic        | `0`     | The maximum segment size supported by the remote index build service. This setting allows users to specify an upper bound on the remote index build service segment builds based on the remote index build service implementation's constraints. This setting is only evaluated if the setting is set to a value greater than `0`. |
