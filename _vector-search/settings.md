...
`index.knn.faiss.efficient_filter.disable_exact_search` | Dynamic | `false` | When set to `true`, disables the exact search fallback that occurs after a Faiss efficient filtered approximate nearest neighbor (ANN) search returns fewer than `k` results. For more information, see [Disabling the exact search fallback]({{site.url}}{{site.baseurl}}/vector-search/filter-search-knn/efficient-knn-filtering/#disabling-the-exact-search-fallback).
