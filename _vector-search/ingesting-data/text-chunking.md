...
In order to use the ingest pipeline, you need to create a sparse index. The `passage_chunk_embedding` field must be of the `nested` type. The `sparse_encoding` field must be with type `rank_features`:
