...
| `console clusters curl source -- "/_cat/indices?v"` | Runs direct API checks against the source |
| `console clusters curl target -- "/_cat/indices?v"` | Runs direct API checks against the target |
...
| `workflow configure edit` | Opens the workflow config in your editor |
...
| Command | Why you use it |
|:--------|:---------------|
| `workflow submit` | Starts the migration workflow |
| `workflow manage` | Primary day-to-day interface for monitoring, approvals, and logs |
| `workflow status` | Shows current workflow state in a non-interactive form |
| `workflow output` | Shows logs across workflow pods |
| `workflow output --follow` | Streams logs live |
| `workflow approve <STEP>` | Approves a gated transition |
| `workflow stop` | Stops a running workflow |

## `workflow manage` is the primary interface
