...
# Playbook: Amazon OpenSearch Service → Amazon OpenSearch Serverless (Vector Search)

This playbook walks through migrating an Amazon OpenSearch Service domain running Elasticsearch 7.10 to an Amazon OpenSearch Serverless vector search collection using Migration Assistant. Every command is copy-paste ready for the following environment:

| Setting | Value |
|---------|-------|
| AWS account | `110105103941` |
| Region | `us-east-2` |
| IAM role | Admin |
| Source | Amazon OpenSearch Service domain (`cluster-dev-source-cluster`, Elasticsearch 7.10) |
| Target | Amazon OpenSearch Serverless collection (`vector-search-target`, VECTORSEARCH) |
| Deployment | "Launch into existing VPC" CloudFormation template |
| Stage | `dev` |

If your environment differs, replace the values above in every command.
...
| Authentication | Basic auth, SAML, IAM | IAM SigV4 only |
...
- You know the domain's VPC endpoint (for example, `https://vpc-cluster-dev-source-cluster-5tzpiblcaesvjap4ritcqcaqne.us-east-2.es.amazonaws.com`).
...
aws opensearch describe-domain \
  --region us-east-2 \
  --domain-name cluster-dev-source-cluster \
...
| Endpoint        | vpc-cluster-dev-source-cluster-...    |
| FGAC            | True                                  |
    ...
    "Rules": [
      {
        ...
        "Resource": ["collection/vector-search-target"]
      }
    ],
    "AWSOwnedKey": true
  }'
...
aws opensearchserverless create-access-policy \
  ...
  --policy '[{
    "Rules": [
      ...
      {
        "ResourceType": "index",
        "Resource": ["index/vector-search-target/*"],
        "Permission": [
          "aoss:CreateIndex",
          "aoss:DeleteIndex",
          "aoss:UpdateIndex",
    ...
    "Principal": [
      "arn:aws:iam::110105103941:role/Admin",
      "arn:aws:iam::110105103941:root"
    ]
  }]'
```
{% include copy.html %}
...
- Your AWS credentials have Admin permissions in account `110105103941`.
...
| `SNAPSHOT_ROLE` | `arn:aws:iam::110105103941:role/migration-eks-cluster-dev-us-east-2-snapshot-role` |
| `EKS_CLUSTER_SECURITY_GROUP` | The security group ID for the new EKS cluster |

The default S3 bucket is `s3://migrations-default-110105103941-dev-us-east-2`.
...
aws opensearchserverless update-access-policy \
  ...
  --policy '[{
    ...
    "Principal": [
      "arn:aws:iam::110105103941:role/Admin",
      "arn:aws:iam::110105103941:root",
      "arn:aws:iam::110105103941:role/migration-eks-cluster-dev-us-east-2-migrations-role",
      "arn:aws:iam::110105103941:role/migration-eks-cluster-dev-us-east-2-snapshot-role"
    ]
  }]'
```
{% include copy.html %}
...
aws opensearch update-domain-config \
  ...
  --advanced-security-options '{
    "Enabled": true,
    "MasterUserOptions": {
      "MasterUserARN": "arn:aws:iam::110105103941:role/migration-eks-cluster-dev-us-east-2-migrations-role"
    }
  }'
```
{% include copy.html %}
...
This replaces any existing master user. If you need to preserve an existing master user, map the Migration Assistant role to the `all_access` backend role via the OpenSearch Security API instead.
{: .warning }
...
console clusters curl source -- "/_snapshot/migration-repo" \
  ...
  -d '{
    "type": "s3",
    "settings": {
      "bucket": "migrations-default-110105103941-dev-us-east-2",
      "region": "us-east-2",
      "role_arn": "arn:aws:iam::110105103941:role/migration-eks-cluster-dev-us-east-2-snapshot-role",
...
```json
{
  "sourceClusters": {
    "source": {
      "endpoint": "https://vpc-cluster-dev-source-cluster-5tzpiblcaesvjap4ritcqcaqne.us-east-2.es.amazonaws.com",
      "allowInsecure": false,
      "version": "ES 7.10",
      "authConfig": {
        "sigv4": {
      ...
      "snapshotInfo": {
        "repos": {
          "migration-repo": {
            "awsRegion": "us-east-2",
            "s3RepoPathUri": "s3://migrations-default-110105103941-dev-us-east-2/aos-to-aoss-snapshots",
            "s3RoleArn": "arn:aws:iam::110105103941:role/migration-eks-cluster-dev-us-east-2-snapshot-role"
          }
        },
        "snapshots": {
          "migration-snapshot": {
...
- **Target** uses `sigv4` with `service: "aoss"` — OpenSearch Serverless requires the `aoss` service name for SigV4 signing.
...
- **No `multiTypeBehavior`** — Elasticsearch 7.10 already uses single-type indices, so no type mapping transformation is needed.
...
7. Update client authentication from basic auth or IAM SigV4 with `service: es` to IAM SigV4 with `service: aoss`.
...
Also widen the `indexAllowlist` to migrate all indices:
...
The capture proxy forwards all requests to the source domain and simultaneously records them to Kafka for replay. The proxy uses SigV4 to authenticate to the source, so your clients can continue using their existing authentication method.
{: .note }
...
Update your client's SigV4 signing from `service: es` to `service: aoss`.
...
```bash
aws s3 rb s3://migrations-default-110105103941-dev-us-east-2 --force
```
{% include copy.html %}
...
3. Be passed to the `_snapshot` API via the `role_arn` setting.
...
### Workflow hits "Consistency Guard" error on resubmit
...
#### `index_not_found_exception` on the target

OpenSearch Serverless does not support the `_cluster/settings` API or automatic index creation via templates in the same way as managed OpenSearch. Verify that metadata migration completed successfully before backfill:
...
#### `security_exception` with "User does not have permissions"
