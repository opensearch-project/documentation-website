...
3. Using the migration console, a point-in-time snapshot is taken. Once the snapshot completes, the Metadata Migration Tool is used to establish indexes, templates, component templates, and aliases on the target cluster. With continuous traffic capture in place, `Reindex-from-Snapshot` migrates data from the source. RFS workers coordinate through a dedicated single-node OpenSearch cluster that manages shard assignments and tracks migration progress. This coordinator is deployed automatically and torn down after backfill completes.
