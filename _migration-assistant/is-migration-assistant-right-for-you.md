...
**Apache Solr 6.x–9.x**: Uses a different migration architecture than Elasticsearch (backfill only). Migration Assistant auto-detects SolrCloud compared to standalone deployments. See [Solr migration]({{site.url}}{{site.baseurl}}/migration-assistant/solr-migration/) for details.
