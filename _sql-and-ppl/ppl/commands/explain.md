...
| `<mode>` | Optional | The explain mode. Valid values are: <br> - `standard`: Displays the logical and physical plan along with pushdown information (DSL). Available in both v2 and v3 engines. <br> - `simple`: Displays the logical plan tree without attributes. Requires v3 engine (`plugins.calcite.enabled` = `true`). <br> - `cost`: Displays the standard information plus plan cost attributes. Requires v3 engine (`plugins.calcite.enabled` = `true`). <br> - `extended`: Displays the standard information plus the generated code. Requires v3 engine (`plugins.calcite.enabled` = `true`). <br><br> Default is `standard`. |
